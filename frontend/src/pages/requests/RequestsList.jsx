...
export default function SellerRequests() {
  ...
  async function loadRequests() {
    ...
    try {
      ...
      const data = await getRequests(params.toString())
  ...
  function canSendOffer(request) {
    // Faqat ochiq va bekor qilingan requestlarga taklif yuborish mumkin
    return request.status === 'ochiq' || request.status === 'bekor_qilindi'
  }
  ...
  function getFilteredRequests() {
    ...
    // Tab bo'yicha filterlash
    if (activeTab === 'open') {
      filtered = filtered.filter(req => req.status === 'ochiq')
    } else if (activeTab === 'accepted') {
      // Qabul qilingan requestlar - offer qabul qilingan requestlar
      ...
      // Rad etilgan requestlar - offer rad etilgan requestlar
      ...
      filtered = filtered.filter(req => req.status === 'bekor_qilindi')
    }
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-3xl font-bold text-gray-900">So'rovlar</h1>
            ...
            <div className="mb-6">
              <div className="border-b border-gray-700 bg-white">
                <nav className="-mb-px border-gray-700 flex space-x-8 bg-white">
                  {[
                    { id: 'all', name: 'Barchasi', count: requests.length },
                    { id: 'open', name: 'Ochiq', count: requests.filter(r => r.status === 'ochiq').length },
                    { id: 'accepted', name: 'Qabul qilingan', count: offers.filter(o => o.status === 'qabul_qilindi').length },
                    { id: 'rejected', name: 'Rad etilgan', count: offers.filter(o => o.status === 'rad_etildi').length },
                    { id: 'cancelled', name: 'Bekor qilingan', count: requests.filter(r => r.status === 'bekor_qilindi').length }
                  ].map((tab) => (
                    <button
                      key={tab.id}
                      ...
                      <div className="ml-4 flex flex-col gap-2">
                        ...
                        {canSendOffer(request) && (!myOffer || myOffer.status === 'rad_etildi') && (
                          <button
                            ...
                            className="bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded-lg transition-colors duration-200 flex items-center justify-center"
                          >
                            <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
